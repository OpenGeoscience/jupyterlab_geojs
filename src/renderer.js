...
const CLASS_NAME = 'output_GeoJS rendered_html';
...
export function register_renderer(notebook) {
  ...
  /* A function to render output of 'application/geojs' mime type */
...
/**
 * Re-render cells with output data of 'application/geojs' mime type
 */
export function render_cells(notebook) {
  /* Get all cells in notebook */
  notebook.get_cells().forEach(cell => {
    /* If a cell has output data of 'application/geojs' mime type */
    if (
      cell.output_area &&
